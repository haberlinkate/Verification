...
void Interpreter::Load(Scanner& in_scanner, string binary_filename) {
  ...
  this->WriteBinary(binary_filename);
  vector<string> compare_vector = this->ReadBinary(binary_filename);
  bool is_verified = this->Verify(compare_vector);

  if(is_verified) {
    Utils::log_stream << "ABLE TO VERIFY ASCII CODE" << endl;
  } else {
    Utils::log_stream << "ERROR : UNABLE TO VERIFY ASCII CODE" << endl;
    exit(0);
  }
 ...
 * Function 'Verify'.
 *
 * This walks through a vector of 16-bit strings and compares them to the 
 * 16-bit strings in memory. It will return true if the strings match. It
 * will return false if the strings do not match.
 *
 * Parameter:
 *   compare_vector - the vector to be compared to memory. 
 *
 * Returns:
 *   a boolean value representing if the ASCII code was verified or not.
**/
bool Interpreter::Verify(vector<string> compare_vector) {
#ifdef EBUG
  Utils::log_stream << "enter Verify\n";
#endif

  //cout << "VERIFY" << endl;
  bool verify = true;
  
  if (memory_.size() == compare_vector.size()) {
    for (int i = 0; i < memory_.size(); ++i) {
      //cout << memory_.at(i) << "  " << compare_vector.at(i) << endl;
      Utils::log_stream << memory_.at(i) << "  " 
                        << compare_vector.at(i) << endl;
      if (memory_.at(i) != compare_vector.at(i)) {
        verify = false;
        break;
      }
    }
 ...
 * Function 'ReadBinary'.
 *
 * This function will open a binary file. It will read each 16-bit piece of 
 * binary code. It will expand it to ASCII code and store it in a vector. 
 *
 * Source: Duncan Buell usingFILE.cc
 *
 * Parameter:
 *   binary_filename - the file to be read from.
 *
 * Returns:
 *   a vector of strings representing the ASCII code.
**/
vector<string> Interpreter::ReadBinary(string binary_filename) {
#ifdef EBUG
  Utils::log_stream << "enter ReadBinary\n";
#endif
  
  //cout << "READING" << endl;
  vector<string> return_vector;
  FILE * fp = fopen(binary_filename.c_str(), "r");

  for (int i = 0; i < memory_.size(); ++i) {
    short binary;
    fread(&binary, 2, 1, fp); 
    int ascii_decimal = binary;
    string ascii_string = globals_.DecToBitString(ascii_decimal, 16);
    return_vector.push_back(ascii_string);
  
    //cout << "BINARY: " << ascii_decimal << endl;
...
#ifdef EBUG
  Utils::log_stream << "leave ReadBinary\n";
#endif
 ...
 *
 * This walks through memory. It will read the ASCII code and convert it to
 * a 16-bit binary number. It will write that number to a binary file.
 *
 * Source: Duncan Buell usingFILE.cc
 *
 * Parameter:
 *   binary_filename - the file to be written to.
**/
void Interpreter::WriteBinary(string binary_filename) {
#ifdef EBUG
  Utils::log_stream << "enter WriteBinary\n";
#endif
  ...
  for (int i = 0; i < memory_.size(); ++i) { 
    string ascii_string = memory_.at(i);
    int ascii_decimal = globals_.BitStringToDec(ascii_string);
    short binary = static_cast<short>(ascii_decimal);
...
#ifdef EBUG
  Utils::log_stream << "leave WriteBinary\n";
#endif
