 ...
 *  bool is_negative_      : is this operand invalid?
 *  bool is_null_          : is this operand null?
 ...
 * This simply returns an error message string if the 'is_invalid' flag
...
void Hex::ParseHexOperand() {
  ...
  // We set the value to zero for invalid input.
  if (text_.length() != 5) {
    value_ = 0;
    is_invalid_ = true;
  } else if (text_ == "     ") {
